"""Baseline 4 training: Baseline 3 plus MD-ensemble representation distillation."""
...
from md_frame_dataset import FRAME_SNAPSHOTS, Baseline3Dataset, collate_baseline3_batch
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train Baseline 4 with representation distillation.")
    ...
    parser.add_argument("--lambda_avg", type=float, default=0.02)
...
def resolve_default_save_dir(args: argparse.Namespace, split_info: dict[str, object]) -> Path:
    ...
    return Path("../results/training_runs") / f"baseline4_{args.split_mode}_{run_name}"
...
def normalize_targets(batch: dict[str, object], normalizer: LabelNormalizer) -> dict[str, torch.Tensor | None]:
    normalized: dict[str, torch.Tensor | None] = {
        "y_exp_norm": normalizer.normalize_exp(batch["y_exp"]),
        "y_avg_pb_norm": normalizer.normalize_avg_pb(batch["y_avg_pb"]),
        "y_frame_pb_norm": None,
    }
    if batch["y_frame_pb"] is not None:
        normalized["y_frame_pb_norm"] = normalizer.normalize_frame_pb(batch["y_frame_pb"])
    return normalized


def compute_training_losses(
    outputs: dict[str, torch.Tensor | None],
    normalized_targets: dict[str, torch.Tensor | None],
    lambda_avg: float,
    lambda_frame: float,
    ...
    mse = nn.MSELoss()
    loss_exp = mse(outputs["pred_exp_norm"], normalized_targets["y_exp_norm"])
    loss_avg_pb = mse(outputs["pred_avg_pb_norm"], normalized_targets["y_avg_pb_norm"])

    pred_frame_pb_norm = outputs["pred_frame_pb_norm"]
    target_frame_pb_norm = normalized_targets["y_frame_pb_norm"]
    if pred_frame_pb_norm is None or target_frame_pb_norm is None or target_frame_pb_norm.numel() == 0:
        loss_frame_pb = torch.tensor(0.0, device=loss_exp.device)
    else:
        loss_frame_pb = mse(
            pred_frame_pb_norm.view(-1, pred_frame_pb_norm.size(-1)),
            target_frame_pb_norm,
        )

    if distill_active:
        z_crystal = outputs["z_crystal"]
        h_teacher = outputs["h_teacher"]
        if z_crystal is None or h_teacher is None:
            raise ValueError("Distillation is active but z_crystal or h_teacher is missing")
        loss_distill = mse(z_crystal, h_teacher.detach())
    else:
        loss_distill = torch.tensor(0.0, device=loss_exp.device)

    weighted_avg = lambda_avg * loss_avg_pb
    weighted_frame = lambda_frame * loss_frame_pb
    weighted_distill = lambda_distill * loss_distill
    loss_total = loss_exp + weighted_avg + weighted_frame + weighted_distill
    diagnostics = {
        ...
        "L_distill": float(loss_distill.detach().cpu().item()),
        "weighted_L_distill": float(weighted_distill.detach().cpu().item()),
        "L_total": float(loss_total.detach().cpu().item()),
    }
    return loss_total, diagnostics


def compute_exp_rmse(pred_values: list[float], true_values: list[float]) -> float:
    if not pred_values:
        return float("nan")
    squared_errors = [(pred - true) ** 2 for pred, true in zip(pred_values, true_values)]
    return math.sqrt(sum(squared_errors) / len(squared_errors))


def print_tensor_shapes(outputs: dict[str, torch.Tensor | None]) -> None:
    h_frames = outputs["h_frames"]
    if h_frames is None:
        raise ValueError("Debug tensor shapes requested but h_frames is missing")

    print("Baseline 4 debug tensor shapes")
    print(f"  h_crystal: {tuple(outputs['h_crystal'].shape)}")
    print(f"  h_200: {tuple(h_frames[:, 0, :].shape)}")
    print(f"  h_250: {tuple(h_frames[:, 1, :].shape)}")
    print(f"  h_300: {tuple(h_frames[:, 2, :].shape)}")
    print(f"  h_350: {tuple(h_frames[:, 3, :].shape)}")
    print(f"  h_400: {tuple(h_frames[:, 4, :].shape)}")
    print(f"  h_teacher: {tuple(outputs['h_teacher'].shape)}")
    print(f"  z_crystal: {tuple(outputs['z_crystal'].shape)}")
    print(f"  pred_exp: {tuple(outputs['pred_exp'].shape)}")
    print(f"  pred_avg_pb: {tuple(outputs['pred_avg_pb'].shape)}")
    print(f"  pred_frame_pb: {tuple(outputs['pred_frame_pb'].shape)}")
    print(f"  pred_exp_norm: {tuple(outputs['pred_exp_norm'].shape)}")
    print(f"  pred_avg_pb_norm: {tuple(outputs['pred_avg_pb_norm'].shape)}")
    print(f"  pred_frame_pb_norm: {tuple(outputs['pred_frame_pb_norm'].shape)}")
...
def train_one_epoch(
    ...
    for raw_batch in loader:
        ...
        normalized_targets = normalize_targets(batch, normalizer)
        optimizer.zero_grad()
        outputs = model(batch["crystal_batch"], batch["frame_batch"], num_frames_per_sample=len(FRAME_SNAPSHOTS))

        if debug and not debug_shapes_printed:
            print_tensor_shapes(outputs)
            debug_shapes_printed = True

        loss, diagnostics = compute_training_losses(
            outputs,
            normalized_targets,
    ...
    return {key: value / total_graphs for key, value in totals.items()}, debug_shapes_printed
...
def evaluate(model, loader, device, normalizer, lambda_avg: float):
    ...
    averaged.update(
        {
            "exp_rmse_kcal": compute_exp_rmse(pred_exp, true_exp),
            "pred_exp": pred_exp,
            "true_exp": true_exp,
...
def main() -> None:
    ...
    model.set_normalization_stats(
        exp_mean=normalizer.exp_mean.to(device),
        exp_std=normalizer.exp_std.to(device),
        avg_pb_mean=normalizer.avg_pb_mean.to(device),
        avg_pb_std=normalizer.avg_pb_std.to(device),
        frame_pb_mean=normalizer.frame_pb_mean.to(device),
        frame_pb_std=normalizer.frame_pb_std.to(device),
    ...
    save_json(
        {
            ...
            "seed": args.seed,
            "debug": debug,
            "frame_order": FRAME_SNAPSHOTS,
            "ablation_purpose": (
                "Baseline 4 is an ablation-style extension testing whether explicit "
                "MD-ensemble representation alignment adds value beyond Baseline 3 "
                "shared-encoder auxiliary learning."
            ),
        },
    ...
    for epoch in range(1, args.epochs + 1):
        distill_active = epoch > args.warmup_epochs
        train_metrics, debug_shapes_printed = train_one_epoch(
            model,
            train_loader,
            optimizer,
            device=device,
            ...
            distill_active=distill_active,
            debug=debug,
            debug_shapes_printed=debug_shapes_printed,
        )
        ...
        train_log_rows.append(
            {
                ...
                "train_L_distill": train_metrics["L_distill"],
                "train_weighted_L_distill": train_metrics["weighted_L_distill"],
                ...
                "eval_exp_rmse_kcal": eval_results["exp_rmse_kcal"],
            }
        )

        model_selection_metric = eval_results["exp_rmse_kcal"]
        if model_selection_metric < best_model_metric:
            best_model_metric = model_selection_metric
            best_epoch = epoch
            ...
            torch.save(
                {
                    ...
                    "best_eval_exp_rmse_kcal": best_model_metric,
                    "normalize_labels": normalize_labels,
                    "warmup_epochs": args.warmup_epochs,
                    "lambda_distill": args.lambda_distill,
                },
        ...
        if epoch == 1 or epoch % args.print_every == 0 or epoch == args.epochs:
            distill_status = "active" if distill_active else "inactive"
            print(
                f"Epoch {epoch:4d} | "
                ...
                f"L_distill={train_metrics['L_distill']:.6f} "
                f"lambda_distill*L_distill={train_metrics['weighted_L_distill']:.6f} "
                f"L_total={train_metrics['L_total']:.6f} "
                f"distill={distill_status} | "
                f"eval_L_exp={eval_results['L_exp']:.6f} "
                f"eval_rmse_kcal={eval_results['exp_rmse_kcal']:.6f}"
            )
    ...
    print()
    print("Baseline 4 training finished")
    ...
    print(f"  best_eval_exp_rmse_kcal: {best_model_metric:.6f}")
    print(f"  test_exp_rmse_kcal: {test_results['exp_rmse_kcal']:.6f}")
