"""Aggregate Baseline 3 run predictions and generate publication-style plots."""
...
try:
    ...
    raise SystemExit(
        "numpy is required for evaluate_baseline3_runs.py. "
...
try:
    ...
    raise SystemExit(
        "matplotlib is required for evaluate_baseline3_runs.py. "
        "Load a conda/module environment first, for example: "
        "'source /etc/profile.d/modules.sh && module load Anaconda3/2024.06-1'."
    ) from exc
...
TARGET_SPECS = [
    ...
    ("polar_solv", "pred_polar_solv", "true_polar_solv"),
    ("nonpolar_solv", "pred_nonpolar_solv", "true_nonpolar_solv"),
    ("dispersion", "pred_dispersion", "true_dispersion"),
    ("total", "pred_total", "true_total"),
]
EXPECTED_SAMPLE_IDS = ["6QLN", "6QLO", "6QLP", "6QLR", "6QLT"]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize Baseline 3 prediction CSVs and plot parity figures.")
    parser.add_argument("--results_root", default="../results/training_runs")
    parser.add_argument("--run_dirs", nargs="*", default=None)
    parser.add_argument("--run_prefix", default="baseline3_")
...
def load_predictions(results_root: Path, run_dirs: list[str]) -> list[dict[str, object]]:
    ...
    for run_dir in run_dirs:
        ...
        for row in file_rows:
            ...
            merged_row: dict[str, object] = {"run_dir": run_dir, "sample_id": sample_id}
    ...
    if sorted(EXPECTED_SAMPLE_IDS) == observed_sample_ids:
...
def compute_summary(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for target_name, pred_key, true_key in TARGET_SPECS:
...
def save_predictions_csv(rows: list[dict[str, object]], output_path: Path) -> None:
    fieldnames = ["run_dir", "sample_id"]
    for _, pred_key, true_key in TARGET_SPECS:
        fieldnames.extend([pred_key, true_key])
...
def save_summary_csv(summary_rows: list[dict[str, object]], output_path: Path) -> None:
    fieldnames = ["target", "rmse", "pearson_r", "n"]
    with output_path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
...
def plot_exp_parity(
    ...
    metrics_text = "\n".join(
        [
            ...
            f"r = {float(exp_metrics['pearson_r']):.2f}",
    ...
    ax.set_title("Baseline 3 Parity Plot for Experimental Binding Free Energy", pad=12)
...
def main() -> None:
    ...
    if not run_dirs:
        raise SystemExit("No Baseline 3 run directories found to summarize.")

    rows = load_predictions(results_root, run_dirs)
    summary_rows = compute_summary(rows)

    merged_path = results_root / "baseline3_merged_predictions.csv"
    summary_path = results_root / "baseline3_summary_metrics.csv"
    plot_path = results_root / "baseline3_exp_parity_plot.png"
    ...
    print("Baseline 3 summary")
    for row in summary_rows:
        print(
            f"  {row['target']}: rmse={float(row['rmse']):.4f} "
            f"pearson_r={float(row['pearson_r']):.4f} n={int(row['n'])}"
        )
