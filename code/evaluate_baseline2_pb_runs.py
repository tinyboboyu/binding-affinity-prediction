"""Aggregate Baseline 2-PB run predictions and generate experimental Delta G diagnostics."""
...
try:
    ...
    raise SystemExit(
        "numpy is required for evaluate_baseline2_pb_runs.py. "
...
try:
    ...
    raise SystemExit(
        "matplotlib is required for evaluate_baseline2_pb_runs.py. "
        "Load a conda/module environment first, for example: "
        "'source /etc/profile.d/modules.sh && module load Anaconda3/2024.06-1'."
    ) from exc
...
TARGET_SPECS = [
    ("exp", "pred_exp", "true_exp", "primary"),
    ("avg_pb_vdw", "pred_avg_pb_vdw", "true_avg_pb_vdw", "auxiliary"),
    ("avg_pb_elec", "pred_avg_pb_elec", "true_avg_pb_elec", "auxiliary"),
    ("avg_pb_polar_solv", "pred_avg_pb_polar_solv", "true_avg_pb_polar_solv", "auxiliary"),
    ("avg_pb_nonpolar_solv", "pred_avg_pb_nonpolar_solv", "true_avg_pb_nonpolar_solv", "auxiliary"),
    ("avg_pb_dispersion", "pred_avg_pb_dispersion", "true_avg_pb_dispersion", "auxiliary"),
    ("avg_pb_total", "pred_avg_pb_total", "true_avg_pb_total", "auxiliary"),
]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize Baseline 2-PB prediction CSVs and plot exp parity.")
    parser.add_argument("--results_root", default=str(DEFAULT_RESULTS_ROOT))
    parser.add_argument("--run_dirs", nargs="*", default=None)
    parser.add_argument("--run_prefix", default="baseline2_pb_")
...
def load_predictions(results_root: Path, run_dirs: list[str], split_filter: str) -> list[dict[str, object]]:
    ...
    for run_dir in run_dirs:
        ...
        for row in file_rows:
            ...
            for _, pred_key, true_key, _ in TARGET_SPECS:
...
def compute_summary(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for target_name, pred_key, true_key, role in TARGET_SPECS:
...
def save_predictions_csv(rows: list[dict[str, object]], output_path: Path) -> None:
    fieldnames = ["run_dir", "sample_id", "split", "abs_error"]
    for _, pred_key, true_key, _ in TARGET_SPECS:
        fieldnames.extend([true_key, pred_key])
...
def save_summary_csv(summary_rows: list[dict[str, object]], output_path: Path) -> None:
    fieldnames = ["target", "role", "mae", "rmse", "pearson_r", "n"]
    with output_path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
...
def plot_exp_parity(rows: list[dict[str, object]], exp_metrics: dict[str, object], output_path: Path, dpi: int) -> None:
    ...
    pearson_text = "nan" if math.isnan(float(exp_metrics["pearson_r"])) else f"{float(exp_metrics['pearson_r']):.2f}"
    metrics_text = "\n".join(
        [
            f"MAE = {float(exp_metrics['mae']):.2f} kcal/mol",
    ...
    ax.set_title("Baseline 2-PB Parity Plot for Experimental Binding Free Energy", pad=12)
...
def main() -> None:
    ...
    if not run_dirs:
        raise SystemExit(f"No Baseline 2-PB run directories found under {results_root}")
    ...
    merged_path = results_root / f"baseline2_pb{suffix}_merged_predictions.csv"
    summary_path = results_root / f"baseline2_pb{suffix}_summary_metrics.csv"
    plot_path = results_root / f"baseline2_pb{suffix}_exp_parity_plot.png"
    ...
    pearson_text = "nan" if math.isnan(float(exp_metrics["pearson_r"])) else f"{float(exp_metrics['pearson_r']):.4f}"
    print("Baseline 2-PB evaluation finished")
    print(f"  split: {args.split}")
    print("  primary experimental Delta G metrics:")
    print(
        f"    mae={float(exp_metrics['mae']):.4f} "
        f"rmse={float(exp_metrics['rmse']):.4f} "
        f"pearson_r={pearson_text} n={int(exp_metrics['n'])}"
    )
    print("  auxiliary PB diagnostics:")
    for row in summary_rows:
        if row["role"] != "auxiliary":
            continue
        aux_pearson = "nan" if math.isnan(float(row["pearson_r"])) else f"{float(row['pearson_r']):.4f}"
        print(
            f"    {row['target']}: mae={float(row['mae']):.4f} "
            f"rmse={float(row['rmse']):.4f} pearson_r={aux_pearson} n={int(row['n'])}"
        )
